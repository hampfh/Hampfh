    ...
    let matches = command!()
        ...
        .subcommand(
            Command::new("match")
                ...
                .arg(Arg::new("path-one").long("path-one").value_name("PATH").help("Path to first bot (.lua)").required(true))
                .arg(Arg::new("path-two").long("path-two").value_name("PATH").help("Path to second bot (.lua)").required(true))
                ...
                .arg(arg!(-o --output <PATH> "Output directory for files").default_value("output").value_parser(value_parser!(String)),)
                .arg(Arg::new("init-timeout").long("init-timeout").value_name("TIMEOUT").help("Time in milliseconds that the script has to initialize").default_value("250").value_parser(value_parser!(u64)))
                .arg(Arg::new("turn-timeout").long("turn-timeout").value_name("TIMEOUT").help("Time in milliseconds that the script has to return a move").default_value("250").value_parser(value_parser!(u64)))
        )
        .subcommand(
            Command::new("db")
